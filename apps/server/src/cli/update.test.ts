...
it.layer(NodeServices.layer)("t3 update launcher", (it) => {
  it.effect.skipIf(!symlinksSupported)(
    "repoints a symlink that lives in a runtime versions tree",
    () =>
      Effect.gen(function* () {
  ...
  it.effect.skipIf(!symlinksSupported)("leaves a plain copy or a foreign symlink alone", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "t3-update-" });
  ...
  it.effect.skipIf(windowsHost)("finds the launcher a bare command name resolved to on PATH", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const root = yield* fs.makeTempDirectoryScoped({ prefix: "t3-update-" });
