...
const SYSTEM_TOOLS = ["ca-certificates", "curl", "git", "git-lfs", "jq", "openssh-client"] as const;
...
export function daytonaWorkerImage(
  ...
  return Image.base(baseImage)
    .runCommands(
      ...
      `npm install --global --omit=dev --no-audit --no-fund t3@${DAYTONA_WORKER_IMAGE_VERSIONS.t3} @openai/codex@${DAYTONA_WORKER_IMAGE_VERSIONS.codex} @anthropic-ai/claude-code@${DAYTONA_WORKER_IMAGE_VERSIONS.claudeCode}`,
