...
## User-visible opener and progress

This create reference is inlined on create starts so the opener is available before any tool call. Before the first tool call, send this message:
...
Send the text inside that block as plain chat text, without the fence, keeping the blank line so the closing sentence is its own paragraph rather than part of item 3. It is verbatim copy: do not paraphrase, translate, reword, reorder, shorten, add an introduction, or merge it with other text.

Immediately after the verbatim message, use `TodoWrite` to create this exact five-item checklist in this order:
...
These five todos are the entire `TodoWrite` list for this workflow. Each title is verbatim user-visible copy: send the exact string above, even when the repository's situation suggests a more accurate or specific description. Do not create additional todos or add, remove, rename, reorder, split, or merge these items. Start `Understand the codebase` as `in_progress` and the other four as `pending`. Keep the checklist updated throughout the run: maintain at most one `in_progress` item, mark an item `completed` only after its work succeeds, and move the next item to `in_progress`. Keep the current item `in_progress` while blocked on required user action so the checklist reflects where work will resume.

If this file is read after work has already started, do not resend the opener or recreate the todos.

Use these meanings internally without adding them to the displayed titles:

- `Understand the codebase`: complete this reference's discovery work, including the effective environment source, repository guidance, manifests, tool versions, canonical development commands, services, and external requirements.
- `Generate setup script`: design and implement the environment, resolve or request any required secrets and network access, run the install twice, start the required services, run the relevant lint/test/build commands, and complete the local hello-world verification. If user action blocks setup, keep this item `in_progress` until the blocker is resolved.
- `Take a snapshot`: take a fresh snapshot of the locally verified VM and wait for it to become ready.
- `Verify build in a subagent`: when build tools are available, trigger the draft build, wait for success, and verify that exact build in a fresh cloud subagent. This is the existing fresh-agent verification in this reference, not a replacement for local verification. If build selection or subagents are unavailable, mark this item `cancelled` and disclose the skipped verification.
- `Verify success and show card`: after the required local validation succeeds, propose the final configuration so the Environment card appears, send the end-of-run summary, and ask the user to Save. This item reports the evidence already collected; it does not add another verification pass and does not depend on fresh-agent verification when that step was unavailable.
...
Use subagents for broad, independent repository analysis when available, but review the files and commands yourself before adopting their recommendations. Carry these findings into the environment design:
...
If Docker must run inside the Cloud Agent VM, account for the nested-container environment. Docker may require `fuse-overlayfs`, legacy iptables, and explicit daemon startup. Validate the current platform instead of pasting a stale package-version recipe.
...
2. When environment-build tools are available, trigger a draft build with `environmentJson` containing that READY `snapshot` plus the install/start scripts under test, including greenfield creates with no linked environment yet. Wait for it to finish, then verify that exact build in a fresh cloud subagent. Asking the user to set up an environment through this skill is the explicit request to run that build-test path; do not trigger builds for unrelated questions.
...
When `trigger-environment-build` is available, call it with `environmentJson` per the tool schema, using the READY `snapshot` from the snapshot step plus the install/start scripts under test. A missing environment ID is fine: passing `environmentJson` creates and links a personal transitional draft automatically. Prefer omitting `refs` so the build uses each repository's default revision unless a non-default source is required for the test.
...
If `cursor-cloud-propose-environment-json` is available, call it after validation succeeds. Use the tool's `environmentJson` and optional `buildId` parameters. When a draft build was tested, pass that build's id as `buildId` so Portal Save reuses the snapshot the build validated.
...
If build tools are unavailable, skip the build-and-verify step entirely. Still snapshot the working VM, then propose scripts without a `buildId`. Do not claim a build was tested.

## 5. Report the result

Finish the five-item checklist to match the work that actually ran: mark finished items `completed` and skipped fresh-agent verification `cancelled`.

Then send the successful Save response below as the user-visible summary of what this create run did. Directly under the opening sentence, add a short bullet list titled **What was done** covering the concrete setup work that succeeded, including discovered stack, install/start, local verification, snapshot, and build/subagent when they ran. Keep it to a few bullets; do not dump logs.
...
- <concrete setup work that succeeded>
