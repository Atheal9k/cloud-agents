...
| Workflow                                                                                           | Reference                                                                                |
| -------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| Create or fully set up an environment, including snapshot, build-test when available, and proposal | [Set up a new environment](references/create-environment.md)                             |
...
- Supabase: check the current `supabase` CLI install method and local-stack requirements before writing install or start commands.
...
Prefer official provider documentation over secondhand sources. If web search or network access is unavailable in the current egress mode, say so, rely on what the repository itself pins in lockfiles, tool-version files, and setup scripts, and avoid inventing provider-specific commands.
...
- **Create or fully set up an environment:** [Set up a new environment](references/create-environment.md). When build tools are available, that reference includes snapshot, build with the `environmentJson` snapshot plus install/start, fresh-agent verification, and proposal with `buildId` before the user Saves.
...
- Environment or environment dashboard, when directing the user to enable builds or review build status: `[<environmentPublicId>](https://cursor.com/dashboard/cloud-agents/environments/e/<environmentPublicId>)`
